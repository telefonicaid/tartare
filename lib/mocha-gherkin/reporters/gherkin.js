...
function GherkinReporter(runner) {
  ...
  runner.on('feature', function(feature) {
    ...
    console.log(indent((feature.pending ? styles.featureTitlePending : styles.featureTitle)(getDecoratedTitle(feature)) + os.EOL));
  ...
  runner.on('it fail', function(test, err) {
    // 'Spec reporter'-like output
    GherkinBase.Base.cursor.CR();
    var fmt = indent(GherkinBase.Base.color('fail', '  ' + GherkinBase.Base.symbols.err + ' %d) %s'));
    console.log(fmt, self.failures.length, test.title);
  });
  ...
  runner.on('end', function() {
    ...
    if (self.warningBuggyTestPassed.length) {
      console.error();
      console.error(styles.failureMessage('*** WARNING: The following variants are marked as bugs but they are passing ***'));
    ...
    if (self.warningNonBuggyTestFailed.length) {
      console.error();
      console.error(styles.failureMessage('*** WARNING: The following variants are failing but they are not marked as bugs ***'));
      console.error(styles.failureMessage('==================================================================================='));
