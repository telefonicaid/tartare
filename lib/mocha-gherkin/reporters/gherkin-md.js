...
function GherkinMdReporter(runner) {
  ...
  var self = this;
  var bugIdPrefix = require('../../utils').getTartareOptions('bugid-link') || null;
  ...
  function printTOC(rootSuite) {
    ...
    rootSuite.suites.forEach(function(feature) {
      ...
      console.log('- [' + feature.title + '](#' + slug(feature._fullTitle()) + ')');
      feature.suites.forEach(function(scenario) {
        if (scenario.type !== 'scenario') {
          return;
        }
        console.log('  - [' + scenario.title + '](#' + slug(scenario._fullTitle()) + ')');
      });
    });
    console.log();
    console.log('---');
    ...
    for (var bugId in bugs) {
      if (bugs.hasOwnProperty(bugId)) {
        ...
        bugs[bugId].forEach(function (suite) {
          console.log('    - [' + suite.title + '](#' + slug(suite._fullTitle()) + ')');
        });
      }
    }
  }
  ...
  runner.on('start', function() {
    ...
    // It also convert suites in "manual" when they have manual children
    ...
    runner.suite.suites.forEach(function(feature) {
      ...
      console.log('<a name="' + slug(feature._fullTitle()) + '"></a>');
      console.log('## ' + title);
      ...
      feature.suites.forEach(function(scenario) {
        ...
        console.log('<a name="' + slug(scenario._fullTitle()) + '"></a>');
        console.log('### ' + title);
        ...
        scenario.suites.forEach(function(variant) {
          ...
          if (!variant.dummy) {
            ...
            console.log('<a name="' + slug(variant._fullTitle()) + '"></a>');
            console.log('**' + title + '**');
          }
