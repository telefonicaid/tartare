  ...
  suite.on('pre-require', function(context, file, mocha) {
    ...
    function _createStep(options) {
      return function(title, fn) {
        ...
        if (test.manual) {
          test.parent.hasManualChildren = test.parent.parent.hasManualChildren = test.parent.parent.parent.hasManualChildren = true;
    ...
    function _createAllHook(options) {
      return function(fn) {
        ...
        hooksArray.slice(-1).subtype = options.type + 'All';
      };
    }
    ...
    function _createFeatureHook(options) {
      return function(fn) {
        ...
        if (!suites[0].pending) {
          suites[0][options.type + 'All'](options.type + 'Feature', fn);
          var hooksArray = suites[0]['_' + options.type + 'All'];
          hooksArray.slice(-1).subtype = options.type + 'Feature';
        }
      };
    }
      ...
      return function(fn) {
        ...
        if (suites[0].suites.length === 0) {
          ...
          suites[0].suites.forEach(function(scenario) {
            if (!scenario.pending) {
              scenario[options.type + 'All'](options.type + 'EachScenario', fn);
              var hooksArray = scenario['_' + options.type + 'All'];
              hooksArray.slice(-1).subtype = options.type + 'EachScenario';
            }
          });
        }
      };
      ...
      return function(fn) {
        ...
        if (suites[0].parent.suites.length === 1) {
          ...
          if (!suites[0].parent.pending) {
            suites[0].parent[options.type + 'All'](options.type + 'Scenario', fn);
            var hooksArray = suites[0].parent['_' + options.type + 'All'];
            hooksArray.slice(-1).subtype = options.type + 'Scenario';
          }
        }
      };
    }
    ...
    function _createEachVariantHook(options) {
      return function(fn) {
        ...
        if (!suites[0].pending) {
          suites[0][options.type + 'All'](options.type + 'EachVariant', fn);
          var hooksArray = suites[0]['_' + options.type + 'All'];
          hooksArray.slice(-1).subtype = options.type + 'EachVariant';
        }
      };
    }
     ...
     *
     * @param optName Option name whose value is wanted (in the lowerCamelCase form).
     *                Without this parameter, this function returns an object with
     *                all the options passed in to Tartare.
     * @returns [String|Object]
     */
    context.getTartareOptions = function getTartareOptions(optName) {
      if (optName) {
        return mocha.tartare.options[optName];
      } else {
        return mocha.tartare.options;
      }
    };
