...
/**
 * Asynchronous wrapper for steps and hook that could execute synced code. It is similar to the asyncIt function
 * from the synchronize module, but using Promises if available because it allows code returning Promises to work.
 *
 * @param {Function} fn - The function to be wrapped.
 * @return {Function}
 * @private
 */
function _asyncIt(fn) {
  if (!fn) {
    throw new Error('No function for _asyncIt');
  }

  var asyncFn;
  // If there are promises (node >=0.11.13 or 3rd party library) use them to async the function so
  // functions returning promises will work.
  if (global.Promise) {
    asyncFn = function() {
...
module.exports = function(rootSuite) {
  ...
  rootSuite.on('pre-require', function(context, file, mocha) {
    ...
    function _skipPending(fn, suite) {
      var wrappedFn;
      if (fn.length) {  // fn comes with a done argument, so it is async
        wrappedFn = function(done) {
          if (suite.isPending()) {
            done();
          } else {
      ...
      } else {  // fn comes with no arguments, so it is sync or is returning a Promise
        wrappedFn = function() {
          if (!suite.isPending()) {
